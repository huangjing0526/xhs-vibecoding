...
import { BookmarkPlus, Download, Loader2, Sparkles, Trash2 } from "lucide-react";
 ...
 * 觉得某张值得反复用，就在大图里「存入资产库」，它才成为下次生成能取用的素材。
 */
...
export default function WorksLibrary({ onNotice }: { onNotice: (notice: Notice) => void }) {
  ...
  const [zoomed, setZoomed] = useState<WorkEntry | null>(null);
  ...
  const handleDelete = async (work: WorkEntry) => {
    try {
  ...
  const templates = useMemo(
    () => groupInOrder(works, (work) => work.templateId).map((group) => ({
      id: group.key,
  ...
  const visible = templateFilter ? works.filter((work) => work.templateId === templateFilter) : works;
    ...
    <div className="mt-6 space-y-4">
      ...
      {templates.length > 0 && (
        <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1">
          <FilterChip label="全部" count={works.length} active={templateFilter === ALL} onClick={() => setTemplateFilter(ALL)} />
          {templates.map((template) => (
            <FilterChip
              key={template.id}
              label={template.name}
              count={template.count}
              active={templateFilter === template.id}
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          title={works.length === 0 ? "还没有作品" : "这个模板还没跑过图"}
          description={
            works.length === 0
              ? "去图片工厂跑一次，产出会自动留在本机，之后都能在这里翻出来、下载或存进资产库。"
              : "换个模板看看，或点「全部」看所有产出。"
          }
        />
      ) : (
        days.map((day) => (
          <Card key={day.key}>
            <h2 className="text-[15px] font-bold text-ink">
              {day.key}
              <span className="ml-2 text-[11px] font-normal text-faint">{day.items.length} 张</span>
            </h2>
...
function downloadName(work: WorkEntry): string {
          ...
          <div className="space-y-3">
            <div className="rounded-2xl bg-soft p-3">
              ...
              <dl className="mt-2 space-y-1.5 text-[11px] leading-5">
                <MetaRow label="生成时间" value={work.createdAt.replace("T", " ").slice(0, 16)} />
