...
豆包 / Dola 网页端的 Chrome / Edge 扩展（Manifest V3），用于提取**你自己生成**的无水印图片、视频资源，支持预览、勾选和批量下载，以及把视频**直接送进内容工作台的视频工厂**挂到指定镜头上。
...
把在豆包生成好的视频直接挂到内容工作台某个视频项目的某一镜上，省掉「下载 → 找文件 → 手动上传」三步。

1. 先把内容工作台跑起来（`npm run dev`，默认 `http://127.0.0.1:3000`）。
2. 在视频工厂里建好项目、拆好分镜。
3. 打开面板，工具栏下方那行「送到工作台」里选项目、填镜号（点「刷新」可重拉项目列表）。
4. 在视频卡片上点「送到工作台」。成功后镜号自动 +1，方便连着送下一镜。

几个说明：

- **只有视频卡片有这个按钮**。图片没有「挂到某一镜」的概念，要图走图片工厂。
- **时长按真实值存**。豆包的片子不是 grok 的 6/10 秒档，面板从预览的 `<video>` 读实际时长一起送过去，工作台不会再吸到 6 或 10。
- **同一镜重复送会覆盖**，和手动上传的行为一致。
- **端口不是 3000 的话**，改 `service-worker.js` 顶部的 `WORKBENCH_ORIGIN`，并把新端口同步加进 `manifest.json` 的 `host_permissions`——只改一处会被扩展权限挡掉。
- 视频字节是**扩展在后台取好再转发**给工作台的：豆包的直链带着会话签名，出了浏览器就取不到，不能让工作台自己去下。

对应的工作台接口是 `POST /api/video-factory/clip`（`provider=doubao`），和手动上传走的是同一个端点。
...
service-worker.js                后台：解析、解密、下载、送片到工作台、开关
...
送片失败时，面板底部那行提示会直接说原因。最常见的是工作台没开——提示会写成「连不上内容工作台」，把 `npm run dev` 跑起来即可。
